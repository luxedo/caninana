...
from .assertions import assert_dataframe_equal_pandas, assert_exception, assert_series_equal_pandas
...
example_list_series_pd = [pd.Series(s.to_list(), index=s.index.to_list()) for s in example_list_series]
example_index = ["d", "e", "f"]
example_columns = [3, 4]
...
class TestDataFrameInit:
    ...
    def test_init_from_collection_of_series(self):
        ...
        pdf = pd.DataFrame(example_list_series_pd)
        assert_dataframe_equal_pandas(df, pdf)
    ...
    def test_init_constructor_empty_data_with_index(self):
        df = lt.DataFrame(index=example_index)
        pdf = pd.DataFrame(index=example_index)
        assert_dataframe_equal_pandas(df, pdf)

    def test_init_constructor_empty_data_with_columns(self):
        df = lt.DataFrame(columns=example_columns)
        pdf = pd.DataFrame(columns=example_columns)
        assert_dataframe_equal_pandas(df, pdf)
    ...
    #         with pytest.raises(ValueError, match=match):
    #             df._validate_index_and_columns()
...
#     # def test_merge(self):
#     #     bananas = {"name": ["bob", "bob", "alice", "carlos", "einstein"], "bananas": [1, 10, 15, 20, 12], "day": [0, 1, 0, 0, 0]}
#     #     ages = {"name": ["bob", "alice", "carlos", "dexter"], "age": [22, 23, 34, 25]}
#     #     dfa = lt.DataFrame(bananas)
#     #     dfb = lt.DataFrame(ages)
#     #     pdfa = pd.DataFrame(bananas)
#     #     pdfb = pd.DataFrame(ages)
#     #     print(dfa.merge(dfb, on=["name"]))
#     #     print(pdfa.merge(pdfb, on=["name"]))
